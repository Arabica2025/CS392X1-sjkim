...
public class Quiz02_01 {
	...
	for (int i = 0; i < n; i +=1){
		...
		for (int j = 0; j < i; j += 1){
			if (xs.getAt(j).compareTo(xs.getAt(i)) < 0 && longest[j] + 1 > longest[i]){
	...
	//indices in FnList<Integer>

	FnList<Integer> resultidx = FnListSUtil.nil();
	while(bestendidx != -1){
		resultidx = FnListSUtil.cons(bestendidx, resultidx);
		bestendidx = prev[bestendidx];
	}
	return resultidx;
	// // to find the leftmost longest ascending subsequence,
	// // iterate through xs
	// 	int longest = 1;
	// 	for(int i = 0; i < xs.length(); i += 1){
	// 		// find the longest subsequence ending at i
	// 		for (int j = i+1; j < xs.length(); j += 1){
	// 			if(xs.getAt(i).compareTo(xs.getAt(j)) < 0){
	// 				longest += 1;
	// 			}
	// 		}
	// 	}
	// another nested loop is needed to find the longest subsequence

    }
    public static void main (String[] args) {
	// HX-2025-11-19:
	// Please write minimal testing code for FnA1szLongestMonoSubsequence
		Integer[] test1 = new Integer[]{5, 1, 2, 3, 4};
		FnA1sz<Integer> xs = new FnA1sz<>(test1);
